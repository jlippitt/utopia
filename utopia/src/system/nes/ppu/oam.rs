...
impl Oam {
    ...
    pub fn set_address(&mut self, value: u8) {
        self.address = value;
        trace!("OAM Address: {:02X}", self.address);
    }

    pub fn read(&self) -> u8 {
        let value = self.primary[self.address as usize];
        trace!("OAM Read: {:02X} => {:02X}", self.address, value);
        value
    }

    pub fn write(&mut self, value: u8) {
        trace!("OAM Write: {:02X} <= {:02X}", self.address, value);
    ...
    pub fn select_sprites(&mut self, line: i32, sprite_size: bool) -> (usize, bool) {
        ...
        for n in 0..=63 {
            ...
            if write_index < self.secondary.len() {
                ...
                if (sprite_y as i32) <= line && (sprite_y as i32 + height) > line {
                    ...
                    if n == 0 {
                        trace!("Line {}: Sprite Zero Selected", line);
        ...
        if write_index > 0 {
            trace!("Line {}: {} Sprites Selected", line, sprites_selected);
        }
